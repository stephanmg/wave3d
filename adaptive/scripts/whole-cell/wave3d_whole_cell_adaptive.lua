...
---[[
synapseInfluxCa:set_error_estimator(eeMult)
synapseInfluxIP3:set_error_estimator(eeMult)
--]]
...
domDisc:add(OneSideP1Constraints())
...
min_dt = util.GetParamNumber("-minDt", 1e-8)
...
for step=0, numSteps do
  -- indicate current time step
  print("Time step # " .. step .. " (" .. (step*dt) .. ")") 
  -- prepare time discretization
  timeDisc:prepare_step(solTimeSeries, dt)

  -- try to solve discrete problem
  ...
  print("ref")
    -- refining
    domDisc:mark_with_strategy(refiner, refStrat)
    ...
    -- coarsening
    domDisc:mark_with_strategy(refiner, coarsStrat)
