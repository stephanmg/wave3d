...
-- ER densities
IP3Rdensity = 17.3
RYRdensity = 0.86 -- was 0.86 before
...
eeCaCyt = SideAndElemErrEstData(4,4,"cyt")
eeCaER  = SideAndElemErrEstData(4,4,"er")
eeIP3   = SideAndElemErrEstData(4,4,"cyt")
eeClb   = SideAndElemErrEstData(4,4,"cyt")
...
diffClb:set_error_estimator(eeClb)
...
diffIP3:set_error_estimator(eeIP3)
...
synapseInflux:set_scaling_factors(1e-9, 1.0, 1.0, 1) -- no 3d/1d coord scaling necessary -> all in units of µm assumed (but MBs example is with meters!)
--synapseInflux:set_scaling_factors(1e-15, 1.0, 1.0, 1.0e-15) -- no 3d/1d coord scaling necessary -> all in units of µm assumed (but MBs example is with meters!)
...
while endTime-time > 0.001*dt do
  ...
  then
    ...
    numElem = dom3d:domain_info():num_elements()
    if refiner:num_marked_elements() > 0 then
      refiner:coarsen()
      refiner:clear_marks()
    end
    ...
    if refinedGrid then
      ...
      VecScaleAssign(u, 1.0, solTimeSeries:latest())
      print("  +++ POINT IN TIME  " .. math.floor((time+dt)/dt+0.5)*dt .. "s   REBALANCING ADAPTED GRID++++")
     -- rebalancing
