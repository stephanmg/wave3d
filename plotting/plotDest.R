...
# destination = soma
destination <- read.csv("meas_meas2rER_ca_er", sep="\t")
# source = neurite tip
source <- read.csv("meas_meas2r_ca_cyt", sep="\t")
...
src <- src + labs(subtitle="Cytosol (At soma)")
src <- src + theme_bw()
...
dest <- dest + labs(subtitle="ER (At soma)")
dest <- dest + theme_bw()
...
ggsave(filename="ca_cyt_vs_er_dest.png", arrangeGrob(src, dest))
