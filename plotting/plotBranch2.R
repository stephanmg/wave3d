...
# destination = soma
destination <- read.csv("meas_meas2lER_ca_er", sep="\t")
# source = neurite tip
source <- read.csv("meas_meas2l_ca_cyt", sep="\t")
...
src <- src + labs(subtitle="Cytosol (Before branching point left)")
src <- src + theme_bw()
...
dest <- dest + labs(subtitle="ER (Before branching point left)")
dest <- dest + theme_bw()
...
ggsave(filename="ca_cyt_vs_er_branch_left.png", arrangeGrob(src, dest))
