...
# destination = soma
destination <- read.csv("meas_meas2ER_ca_er", sep="\t")
# source = neurite tip
...
src <- src + labs(subtitle="Cytosol (At neurite tip)")
src <- src + theme_bw()
...
dest <- dest + labs(subtitle="ER (At neurite tip)")
dest <- dest + theme_bw()
...
ggsave(filename="ca_cyt_vs_er_source.png", arrangeGrob(src, dest))
