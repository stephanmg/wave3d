...
for (i in 4:(4+a-1)) {
  ...
  data <- data.frame(Time=data$Time, Concentration=data$Concentration+2.5e-6*(j), Refinement=refinement)
  cyt <- rbind(cyt, data)
  j <- j + 1
}
...
for (i in (4+a):(length(args))) {
 ...
 # print(args[i])
  colnames(data) <- c("Time", "Concentration")
  refinement <- rep(factor(j), length(data))
  data <- data.frame(Time=data$Time, Concentration=data$Concentration+2.5e-7*(j), Refinement=refinement)
  er <- rbind(er, data)
  j <- j + 1
}
...
endoplasmic_reticulum <- endoplasmic_reticulum + labs(subtitle="endoplasmic reticulum (At neurite tip)")
...
# cytosol
tikz(file=paste(sub('\\.[^\\.]*$', '', outputname), "_cytosol", ".tex", sep=""))
print(cytosol)
dev.off()

# ER 
tikz(file=paste(sub('\\.[^\\.]*$', '', outputname), "_er", ".tex", sep=""))
print(endoplasmic_reticulum)
dev.off()
