...
# destination = soma
destination <- read.csv("meas_meas4ER_ca_er", sep="\t")
# source = neurite tip
source <- read.csv("meas_meas4_ca_cyt", sep="\t")
...
src <- src + labs(subtitle="Cytosol (After branching point: 40 µm)")
src <- src + theme_bw()
...
ggsave(filename="ca_cyt_vs_er_after_branch2.png", arrangeGrob(src, dest))
