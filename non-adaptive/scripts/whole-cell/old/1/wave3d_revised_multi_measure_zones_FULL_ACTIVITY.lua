...
cytVol = "cyt, meas1, meas2" -- soma
erVol = "er, meas1ER, meas2ER, meas2rER" -- inner
plMem = "pm"
plMem_vec = {"pm"}
erMem = "erm"
erMemVec = {"erm"}
bdry = "meas2, meas2r, meas2rER"
...
if generateVTKoutput then 
  ...
  local vtkObserver = VTKOutputObserver(outDir .. vtkFolder .. "/solution", out, pstep)
  limex:attach_observer(vtkObserver)
end

-- output for measurement
measFolder = util.GetParam("-suffix", "vtk/meas/")
measOutName = util.GetParam("-measSuffix", "meas")

-- monitors the source activity
function measSourceActivity(step, time, dt)
  curSol = measSourceObserver:get_current_solution()
  -- at source (cytosol and ER calcium)
  take_measurement(curSol, time, "meas2", "ca_cyt", outDir.. measFolder .. measOutName)
  take_measurement(curSol, time, "meas2ER", "ca_er", outDir.. measFolder ..  measOutName)
  -- 20 micron away from source downstream (before first BP)
  take_measurement(curSol, time, "meas1", "ca_cyt", outDir.. measFolder .. measOutName)
  return take_measurement(curSol, time, "meas1ER", "ca_er", outDir.. measFolder ..  measOutName)
end
...
function measWaveActivity(step, time, dt)
  ...
  -- at soma
 take_measurement(curSol, time, "meas2r", "ca_cyt", outDir.. measFolder .. measOutName)
 return take_measurement(curSol, time, "meas2rER", "ca_er", outDir.. measFolder .. measOutName)
end
...
if (generateVTKoutput) then 
  out:write_time_pvd(outDir .. vtkFolder .. "/solution", u)
end
