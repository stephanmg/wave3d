...
-- load domain
reqSubsets = {"cyt", "er", "pm", "erm", "meas#1", "meas#2", "meas#3", "meas#4", "meas#5"}

dom = util.CreateDomain(gridName, numRefs, reqSubsets)

cytVol = "cyt, meas#2, meas#4"
erVol = "er"
plMem = "pm"
plMem_vec = {"pm"}
erMem = "erm"
erMemVec = {"erm"}
bdry = "meas#1, meas#3, meas#5"
...
-- synaptic activity
synapseInfluxCa = UserFluxBoundaryFV1("ca_cyt", "meas#5")
synapseInfluxCa:set_flux_function("synCurrentDensityCa")
synapseInfluxIP3 = UserFluxBoundaryFV1("ip3", "meas#5")
...
function measWaveActivity(step, time, dt)
	...
	local measConc = take_measurement(curSol, time, "meas#4", "ca_cyt", outDir.."meas/tt.txt")
	if measConc > 4*ca_cyt_init then
		if maxconc == 0 then
			Tbegin = time
			frontconc = measConc
		end
		if measConc > maxconc then
			maxconc = measConc
			Tmax = time
		end
		Tend = time
